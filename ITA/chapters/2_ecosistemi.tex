...
lunghezza del buffer \textit{mem chunk: 0, 1}, che in questo caso a run time a microfoni chiusi.
...
che a run time e a microfoni chiusi partono da costanti distribuite di 0 e 1.
...
la primitiva \textit{rdtable}, che può essere utilizzata per leggere una tabella di sola lettura (predefinita prima del compile time),
e la primitiva \textit{rwtable}, che può essere utilizzata per implementare una tabella di lettura/scrittura; 
quest'ultima prende in input un segnale che può essere scritto nella tabella utilizzando un indice di scrittura, e letto utilizzando un indice di lettura.
Per implementare in Faust l'architettura richiesta in partitura si possono sincronizzare gli indici di scrittura con un 
unico segnale destinato alla scrittura utilizzato dai \textit{sample read} e dai \textit{granular sampling}, e impostare
una grandezza comune, in questo caso di una costante var1 in secondi, 
condivisa dai buffer in questione.
In Faust la grandezza della tabella deve essere espressa in campioni, per ottenere
una grandezza in secondi bisogna dunque moltiplicare il numero di secondi desiderati per
la frequenza di campionamento, definita anche questa come costante (predefinita prima del compile time).
Infine per completare le operazioni come richiesto in partitura si può sostituire l'oggetto \textit{rwtable}
con un send del segnale destinato ad andare in ingresso in feedback ai \textit{sample read} e in
feedforward ai \textit{granular sampling}.
A seguito una implementazione del campionatore come richiesto in partitura in Faust.
\clearpage 
...
\begin{lstlisting}
    ...
    with {
        y = it.frwtable(3, L, .0, writePtr, x, readPtrLen) * 
            trapWin(100) * zeroChunk
        with {
            L = bufferLength * SampleRate; 
                // SR hard-coded: change this to match your samplerate
            writePtr = + (1') %(L) ~ _;
