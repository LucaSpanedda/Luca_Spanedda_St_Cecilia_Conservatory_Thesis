...
che consiste in un Banco di Filtri Bandpass dove per il termine in ingresso \( x\left(n\right) \)  
il segnale viene diviso per il numero di banchi di filtri e restituito in uscita come 
\( y & = x_{{1}}(n), x_{{2}}(n), x_{{3}}(n),... \) .
Assumento ancora una volta C come vettore delle costrizioni e fBank come la funzione
che rappresenta il banco di filtri avremmo infine che:
